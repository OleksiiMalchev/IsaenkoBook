    ...
    (() => {
        ...
        function initSliders() {
            if (document.querySelector(".reviews__slider")) new core(".reviews__slider", {
                modules: [ Navigation, Pagination, Manipulation, EffectCoverflow ],
                speed: 1e3,
                breakpoints: {
                    320: {
                        slidesPerView: "auto",
        ...
        function pageNavigation() {
            ...
            function pageNavigationAction(e) {
                if ("click" === e.type) {
                    const targetElement = e.target;
                    if (targetElement.closest(".goto")) {
                        const gotoLink = targetElement;
                        const gotoLinkSelector = gotoLink.getAttribute("href");
        ...
        if (audioButton) {
            ...
            window.addEventListener("click", (event => {
                if (!audioButton.contains(event.target)) pauseAudio();
            }));
            audioElement.src = audioButton.dataset.audioSrc;
        }
        window["FLS"] = true;
        isWebp();
        menuInit();
